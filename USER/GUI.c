...
{         	
	...
	LCD_SetWindows(x,y,x+size*len-1,y+size-1);	  
	for(i=0;i<x;i++)
	{
		for(j=0;j<y;j++)
		LCD_WR_DATA_16Bit(BLACK);	//写入数据 	 
	}
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,BACK_COLOR,POINT_COLOR,' ',size,1);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,BACK_COLOR,POINT_COLOR,temp+'0',size,1); 
	}
} 
